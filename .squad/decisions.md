...
**Summary:** Root-cause diagnosis and fix for perpetually UNHEALTHY redis health check and orphaned AppHost process trees. **Primary finding:** `ContainerLifetime.Persistent` broke DCP proxy endpoint allocation. Result: `ConnectionStringAvailableEvent` never fired → health check `connectionString` stayed `null` indefinitely → threw `InvalidOperationException` on every probe. **Fix:** Changed `ContainerLifetime.Persistent` → `ContainerLifetime.Session` in `lucia.AppHost/AppHost.cs` line 24. Session lifetime restores `IsProxied=true` → DCP allocates proxy port → `AreResourceEndpointsAllocated` returns true → event fires → health check passes. Data preserved via `WithDataVolume()` (named volume survives restart). **Secondary finding:** Three stale `dotnet watch --project lucia.AppHost` process trees from developer sessions (dated 2026-07-19 and 2026-07-20) were still alive, each holding 435–479 MB working set (~1.63 GB aggregate). Each orphaned tree independently health-polled redis, multiplying the retry storm. Cleaned via `Stop-Process -Id <pid>` (no code change). **Transitive dependency pins:** Locked MessagePack 2.5.198→2.5.302, Microsoft.OpenApi 2.0.0→2.7.5, SQLitePCLRaw.lib.e_sqlite3 2.1.11→3.50.3 in Directory.Packages.props (three GHSA vulnerabilities, NU1902 advisories). **Measurements (post-fix):** Redis container ✓ Healthy, AgentHost ✓ Running, clean AgentHost baseline: 17 MB working set, 53 MB private memory, no sustained 100% CPU. **Prevention:** Use `aspire stop` for clean shutdown; verify no orphaned `dotnet watch` trees with `Get-CimInstance Win32_Process | Where-Object CommandLine -match 'dotnet watch'`.


### 31. Health Check Output Caching & Process Metrics (Hicks, 2026-07-20)

**Summary:** Aspire orchestration floods health-check requests at internal hardcoded intervals (no public knob in 13.1.0). Solution: (1) **Output Cache:** `AddServiceDefaults()` calls `builder.Services.AddOutputCache()` with a named 10-second policy; `MapDefaultEndpoints()` includes `app.UseOutputCache()` middleware. Both `/health` and `/alive` endpoints tagged `"health-checks"` (scoped policy, no caching of unrelated GETs). Cache duration 10s balances cost vs. responsiveness — if a dependency flips unhealthy, next probe hits the endpoint within 10s; if it recovers, cached "unhealthy" expires within 10s. (2) **Set-Cookie/auth safeguard:** Responses with Set-Cookie or auth tokens (authenticated 503s) are excluded from cache by policy, preventing auth-token replay attacks. (3) **Process Metrics:** Added `.AddProcessInstrumentation()` to OpenTelemetry metrics builder; exposes `process.runtime.dotnet.cpu.usage`, `process.runtime.dotnet.memory.working_set`, `.gc.heap.size`, `.gc.allocations.size`, `.gc.collections`, `.gc.heap.fragments.size`, `.threads.count`, `.exception.count`. Already packaged in `OpenTelemetry.Instrumentation.Process` (existing dependency); Aspire dashboard natively consumes these metrics. (4) **Backward compatibility:** Health endpoint contract unchanged (`/health`, `/alive`, `live` tag preserved); both AgentHost and A2AHost (via ServiceDefaults) benefit automatically. **Test coverage:** Three HTTP behavioral tests pass (healthy 200, unhealthy 503, Set-Cookie skip); slopwatch clean. **Impact:** Health check executes once per 10-second window instead of per probe interval (e.g., if Aspire probes every 2s, ~6 runs in 5 minutes instead of 150); Aspire dashboard now displays CPU/memory/GC metrics per service for leak detection and incident diagnostics.

...
### 2026-07-17: Jetson Native Inference Boundary — End-to-End Flow Trace

**Branch:** seiggy-research-jetson-voice-stack (research only)

**Wyoming flow end-to-end (confirmed by source):**

1. `WyomingServer` (IHostedService) listens on TCP port 10400. Enforces `MaxWakeWordStreams` (30) and `MaxConcurrentSttSessions` (4) via a `SemaphoreSlim`.
2. Each TCP connection becomes a `WyomingSession`. State machine: `Connected → WakeListening → Transcribing → Processing → Responding → Connected`.
3. Wyoming protocol events (newline-delimited JSON header + optional extra data + binary payload): `Describe → Info`, `Detect → [AudioChunks → AudioStop/TranscribeEvent] → Detection/Transcript`, `Synthesize → Error`.
4. Per `AudioChunkEvent`: PCM bytes → `ConvertAudioChunkToMonoSamples` (16kHz mono float32) → optional `ISpeechEnhancerSession.Process()` (GTCRN ONNX) → `ISttSession.AcceptAudioChunk()` + `IVadSession.AcceptAudioChunk()`.
5. On `AudioStopEvent`: `IVadSession.Flush()` → `ISttSession.GetFinalResultAsync()` → optional re-transcription of GTCRN-enhanced clip → `IDiarizationEngine.ExtractEmbedding()` + profile match → `TranscriptEvent` sent to HA → background `ITranscriptStore.SaveAsync()`.
6. Speaker detection path: `SpeakerVerificationFilter` → `UnknownSpeakerTracker` → `AdaptiveProfileUpdater`.
7. mDNS advertisement via `ZeroconfAdvertiser` (Makaretu.Dns). `_wyoming._tcp` service type. Instance name and InfoEvent ASR name both come from `WyomingOptions.ServiceName`.

**Inference engines (all singletons, hot-reload via `IModelChangeNotifier`):**
- `SherpaSttEngine` / `HybridSttEngine` — sherpa-onnx `org.k2fsa.sherpa.onnx` v1.12.34 (P/Invoke to native)
- `SherpaVadEngine` — sherpa-onnx silero VAD
- `SherpaWakeWordDetector` — sherpa-onnx KWS zipformer
- `SherpaDiarizationEngine` — sherpa-onnx speaker embedding extractor
- `GtcrnSpeechEnhancer` — `Microsoft.ML.OnnxRuntime` direct (InferenceSession, GTCRN model)
- `GraniteOnnxEngine` — OnnxRuntime direct (IBM Granite 4.0 Speech, 3-model architecture)

**ExcludeSpeech build flag:** Already used by `Dockerfile.agenthost-jetson`. When `true`, all sherpa-onnx and OnnxRuntime engine classes are `Compile Remove`d; `WyomingServer`/`WyomingSession` are also removed. Command routing, APIs, and all data layer stay. This is the current Jetson baseline.

**Narrowest replacement seam:** `ISttEngine` / `ISttSession`. Adding `JetsonSttEngine : ISttEngine` and `JetsonSttSession : ISttSession` with `NativeLibrary.Load` → P/Invoke is the minimum change to run GPU-accelerated STT. No changes to `WyomingServer`, `WyomingSession`, any API, any test, any protocol behavior.

**What C# fully owns without managed ONNX:**
- TCP server + Wyoming protocol (WyomingServer, WyomingSession, parser/writer)
- Session state machine, audio buffering, PCM conversion
- Speaker profile coordination, enrollment, provisional profile management
- Command routing (CommandPatternRouter, SkillDispatcher)
- Transcript storage, model management APIs, all REST+SSE endpoints
- mDNS advertisement

**What requires native on Jetson:**
- Actual GPU inference — `Microsoft.ML.OnnxRuntime.Gpu.Linux` does NOT ship a Jetson TensorRT EP; needs JetPack-native ORT or direct TRT
- CUDA/TensorRT dispatch is fundamentally native

**Boundary decision:** Prefer in-process P/Invoke via `NativeLibrary.Load`. Escalate to out-of-process UDS/gRPC only if the native SDK uses `terminate()` on CUDA errors or if there is a TensorRT context conflict. Full Wyoming service (separate process) only if Brett's stack is Python-only.

**Key files unchanged by Jetson migration:**
- `lucia.Wyoming/Wyoming/WyomingServer.cs`
- `lucia.Wyoming/Wyoming/WyomingSession.cs`
- `lucia.Wyoming/Wyoming/WyomingOptions.cs`
- `lucia.Wyoming/Wyoming/WyomingServiceInfo.cs`
- All `lucia.AgentHost/Apis/Wyoming*Api.cs`
- All test files in `lucia.Tests/Wyoming/`

**Tests that define the interface contracts (and must remain green):**
- `WyomingSessionIntegrationTests.cs` — uses TestSttEngine/TestSttSession
- `WyomingProtocolComplianceTests.cs` — protocol behavior, engine-agnostic
- `WyomingEngineReadinessTests.cs` — `IsReady` gating behavior
- `HybridSttSessionDisposalRaceTests.cs` — ISttSession disposal safety
- `SpeakerVerificationComponentTests.cs` — diarization pipeline with TestDiarizationEngine
- `PostSttPipelineTests.cs` — transcript-to-agent pipeline

**Decision file:** `.squad/decisions/inbox/parker-jetson-native-boundary.md`
...
Research contribution complete. Traced Wyoming host-to-native boundary end-to-end; confirmed in-process P/Invoke (Family C) as preferred over new `JetsonSttEngine`/`JetsonSttSession`. Input consolidated into Decision 27 by Scribe. **Managed code unchanged.** Only native-lib sourcing and GPU-enablement required. Rejected proposals: (1) new engine classes (HybridSttEngine already parametrizes provider); (2) modified Wyoming contracts; (3) separate processes. Kill gates and interface contracts documented for PoC validation on physical Orin Nano 8GB.

...
internal static class ChatHistoryExtensions
{
        ...
        Func<ChatMessageContent, bool>? filter = null)
    {
        ...
        for (var index = startIndex; index <= endIndex; ++index)
        {
            ...
            if (filter?.Invoke(messageContent) ?? false)
            {
                continue;
            }
    ...
    public static void AddSystemMessageWithTokenCount(this ChatHistory chatHistory, string content, Tokenizer tokenizer)
    {
    ...
    /// <param name="chatHistory">Chat history instance</param>
    /// <param name="content">Message content</param>
    public static void AddUserMessageWithTokenCount(this ChatHistory chatHistory, string content, Tokenizer tokenizer)
    {
        IReadOnlyDictionary<string, object?> metadata = new Dictionary<string, object?>
        {
            ["TokenCount"] = tokenizer.CountTokens(content)
    ...
    /// <param name="chatHistory">Chat history instance</param>
    /// <param name="content">Message content</param>
    public static void AddAssistantMessageWithTokenCount(this ChatHistory chatHistory, string content, Tokenizer tokenizer)
    {
        IReadOnlyDictionary<string, object?> metadata = new Dictionary<string, object?>
        {
            ["TokenCount"] = tokenizer.CountTokens(content)

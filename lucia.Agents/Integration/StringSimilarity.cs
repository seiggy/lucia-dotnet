...
public static class StringSimilarity
{
    ...
    public static double TokenCoreSimilarity(string a, string b)
    {
        ...
        var scoreAtoB = AverageBestMatch(coreA, coreB);
        var scoreBtoA = AverageBestMatch(coreB, coreA);

        // Harmonic mean penalizes asymmetric matches more than arithmetic mean.
        // "zack" → "back" scores 0.75 in one direction, but
        // "garage" → "zack" scores low in the other, pulling the result down.
        return 2.0 * scoreAtoB * scoreBtoA / (scoreAtoB + scoreBtoA);
    }

    private static double AverageBestMatch(string[] source, string[] target)
    {
        double total = 0;
        foreach (var s in source)
        {
            ...
            {
                var sim = NormalizedLevenshtein(s.AsSpan(), t.AsSpan());
    ...
    /// and entity keys are pre-cached on <see cref="LightEntity"/>.
    /// </summary>
    public static double HybridScore(
        ...
        double disagreementPenalty = 0.4)
    {
        ...
        var max = Math.Max(Math.Max(levenshtein, tokenCore), phonetic);
        var mean = (levenshtein + tokenCore + phonetic) / 3;
        var spread = max - mean;
        ...
        var stringSimilarity = max - (spread * disagreementPenalty);
